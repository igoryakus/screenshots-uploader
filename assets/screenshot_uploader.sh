...
REMOTE_DIR="/var/www/html/screenshots/files/"
SSH_KEY="/Users/igoryakus/.ssh/vpn_91.149.219.31"
BASE_URL="http://91.149.219.31/screenshots/"
...
process_file() {
    ...
    if [[ "$base" =~ _[a-f0-9]{10}\.[a-zA-Z0-9]+$ ]]; then
        return
    fi
    ...
    salt=$(echo "$(date +%s)$RANDOM" | md5 | cut -c 1-10)
